...
Copy `backend/.env.example` to `backend/.env` and replace placeholders with secure values.
...
**Terminal 1 - FastAPI Server (development):**
```bash
...
# Run container
# Use a production env file and avoid reload mode
docker run -d \
  --name api-optimizer \
  -p 8000:8000 \
